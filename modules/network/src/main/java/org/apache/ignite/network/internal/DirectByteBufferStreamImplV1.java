...
import static org.apache.ignite.network.internal.GridUnsafe.BIG_ENDIAN;
import static org.apache.ignite.network.internal.GridUnsafe.BYTE_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.CHAR_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.DOUBLE_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.FLOAT_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.INT_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.LONG_ARR_OFF;
import static org.apache.ignite.network.internal.GridUnsafe.SHORT_ARR_OFF;
...
public class DirectByteBufferStreamImplV1 implements DirectByteBufferStream {
    ...
    /** */
    private MessageDeserializer<NetworkMessage> msg;
     ...
     */
    public DirectByteBufferStreamImplV1(SerializerProvider msgFactory) {
        ...
        if (msg != null) {
            if (buf.hasRemaining()) {
                try {
                    ...
                    MessageSerializer<NetworkMessage> serializer = msgFactory.createSerializer(msg.type());
    ...
    @Override public <T extends NetworkMessage> T readMessage(MessageReader reader) {
        if (!msgTypeDone) {
            ...
            msg = type == Short.MIN_VALUE ? null : msgFactory.createDeserializer(type);
        ...
        if (msg != null) {
            try {
                ...
                lastFinished = msg.readMessage(reader);
            }
            finally {
        ...
        if (lastFinished) {
            NetworkMessage msg0 = msg.getMessage();
